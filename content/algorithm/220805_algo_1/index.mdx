...
우선, 비교를 위해 스택에 MAX값을 가진 원소를 push한다.

이후 배열을 순회하며 스택의 맨 위 값과 현재 배열이 가리키는 값을 비교하는데,

현재 배열의 값이 더 큰 경우, 스택에서 더 현재 값보다 더 큰 값이 나올 때까지 pop 연산을 수행한다.

스택의 값이 더 큰 경우 해당 값의 인덱스를 정답 배열에 저장한다.
...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		stack.push(new int[] { 987654321, 0 });
		for (int i = 0; i < N; i++) {
			while (stack.peek()[0] < arr[i][0]) {
				stack.pop();
			}
			arr[i][1] = stack.peek()[1];
			stack.push(new int[] { arr[i][0], i + 1 });
		}
